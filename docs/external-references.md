...
The provider client resolves a member with Mailchimp's canonical MD5 subscriber hash of the normalized email. The one-Person synchronization flow first requires an existing BUSINESS Person with a usable primary email and an explicit CRM email preference of `OPTED_IN`; missing/`UNKNOWN` and `OPTED_OUT` preferences do not call Mailchimp. A missing member for an opted-in Person is created with `status=subscribed`, and only the CRM-owned email plus `FNAME`/`LNAME` fields are sent. An existing `subscribed` member may receive those same fields. `unsubscribed`, `cleaned`, `pending`, `transactional`, `archived`, and any other non-subscribed status is protected: the member is not updated, resubscribed, deleted, or otherwise changed. Mailchimp status is not imported as CRM consent.
