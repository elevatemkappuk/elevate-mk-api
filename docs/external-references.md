...
`BREVO_MARKETING_LIST_ID` is an explicit backend environment setting. The manual command `python manage.py sync_brevo_marketing_person <person_id>` uses that configured list and never creates, renames, or deletes lists. Automatic Brevo jobs are not enabled, and MarketingPreference changes continue to enqueue/process only the existing Mailchimp jobs.
