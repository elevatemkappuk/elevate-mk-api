...
The provider client resolves a member with Mailchimp's canonical MD5 subscriber hash of the normalized email. A missing member is created with the POST payload `status=pending`, never `subscribed`, so CRM membership is not treated as marketing consent. An existing `subscribed` member may receive only the CRM-owned email and `FNAME`/`LNAME` fields. `unsubscribed`, `cleaned`, `pending`, `transactional`, `archived`, and any other non-subscribed status is protected: the member is not updated, resubscribed, deleted, or otherwise changed.
