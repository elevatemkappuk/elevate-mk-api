...
For normal operation, run `python manage.py process_brevo_sync_jobs --watch` as a separate worker process. It polls the durable BREVO queue using `BREVO_SYNC_WORKER_POLL_SECONDS` (default `3`) and `BREVO_SYNC_WORKER_BATCH_SIZE` (default `20`, maximum `100`), handles SIGINT/SIGTERM, and never consumes Mailchimp jobs. The web request only commits the preference and enqueues work; it does not wait for Brevo.

Inbound Brevo marketing unsubscribe handling is now implemented at `POST /api/v1/webhooks/brevo/marketing/`. It requires the separately configured Brevo webhook Basic credentials, accepts only `event=unsubscribe`, resolves one BUSINESS Person by exact normalized email, and records source `BREVO` as EMAIL `OPTED_OUT`. Campaign unsubscribe payloads may omit `list_id`; when supplied, it must include `BREVO_MARKETING_LIST_ID`, while an omitted list context requires a reliable event timestamp. Because Brevo's marketing webhook `id` is the internal webhook ID rather than a per-event ID, durable replay receipts use a privacy-preserving event fingerprint. Provider-originated mutations preserve history/audit evidence while suppressing the outbound BREVO echo job. Transactional Brevo webhooks and all other marketing events remain out of scope.
